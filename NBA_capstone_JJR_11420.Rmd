---
title: 'NBA Player Data Analysis '
...
Summary
The National Basketball Association (NBA) is a men’s professional basketball league in North America. It is one of the four major professional sports leagues in the United States and Canada and is widely considered to be the premier men’s professional basketball league in the world. 
The data source is https://www.basketball-reference.comProfessional , providing basketball statistics for every season from Abdul-Jabbar to Zaslofsky and statistics for all NBA players, teams, or leagues

I’m not a Basketball fan, i know the basics of the game and the names of some NBA stars like Michael Jordan and Lebron James. I had the chance to see a play-off match in the Miami Arena , this was really an amazing show. Nothing similar in Europe,it was two and a half hours of games, advertising, musics, songs and, of course, beers and sausages.

For this data science project,i've just want to play on NBA data available and play around with it on my own. However, in my internet searching I  come across lots of similar research made by fans or real experts of the game. The goal of the project is using my current data science knowledge around the players statistics available. 

1- Data source
The website Basketball-Reference.com is an excellent source of the data , the first step is to dowload all website data into a readable and standard format.

2- Data cleaning
The next step is to clean the data , removing N/a and filter them by number of games played.

3-Data analysis
Using all the data available, provide some analyse and statitistics based on player attibutes and positions.

4- Machine learning

Using PCA, reduce the player data in 2 dimensions based on player abilities, the idea is to split players in several clusters.

5-Cluster Analysis

For each cluster found, analyse the players within , are those clusters realistics. Does this reflect the current player positions ?

6- Data prediction

Using RMSE, trying to get the PER of the players, using their pricipal abilities by minutes playes, without knowing the PER formula.

---
                                                     NBA Players Positions Definition:
...
Load all libraries in order to run the code.
Load function to extract data per season from website (map_dfr), join all datas by player  in one file
Load function to convert height and weight in centimeter and kilogram

---

```{r NBA loade,error=TRUE}
...
Check if the datas are already saved in csv in the workong directory.

If yes, load all csv files containig data per decade

If not,

Loading all data from website BASKETBALLREFERENCE.COM
  - Players stats by season
  - Players advanced statistics by season
  - Player general caracteristics by alphabetical order
...
```{r Load NBA data from BasketBallReference ,error=TRUE}
...
#
# research all players in alphabetical; no x in databae
#
#
alphabet<-letters[seq( from = 1, to = 26 )]  
# no more needs to re,ove X  
...
#
#
#  i keep the row data in player_stats and create a working set NBA
#
#
player_data$Player <- gsub("\\*$", "", player_data$player)
player_data<-player_data%>%filter(!is.na(wt) &!is.na(ht))%>%select (player,pos,from,ht,wt)
player_data_r<-player_data%>%filter(!is.na(wt) &!is.na(ht))%>%rowwise()%>%mutate(p_cm=(c_height(ht)),p_kg=(c_weight(wt)))
player_d<-player_data_r%>%select(player,p_cm,p_kg)
#
...
Provide some data cleaning
    - keeping only player with a name
    - remove TOT row for player with 2 teams in same season
    - remove the * for all of fame


```{r Load NBA data cleaning }
...
For convenience: Players Height and Weight convert to Cm and Kg
Does size and Weight matters ? 
Plot data by physical caracteristics, positions
---

```{r statisticsv1,echo=TRUE}
#
# Player statitics - converted height and weight in cm and kg
#
#
...
p_stat%>%group_by(player) %>% summarise(avg_h=mean(p_cm), avg_p=mean(p_kg))
head(p_stat)
...
j %>%
     ggplot(aes(x=as.numeric(player),y=as.numeric(avg_h))) + 
...
I limit the analysis on data after 2017, using the current active NBA players, filter by 50 games played

Find a correlation inside several abilities :
  fg,x3p,x2p,orb,drb,ast,blk
  
Plot the abilities by player positions
---

1
```{r statistics for last season ,echo=TRUE}
#
# Players last year
#  Points by position
#
NBA20<-NBA%>% filter(year>2017)filter(g>50)%>%
NBA20%>%group_by(pos) %>%
  summarise(Games=mean(g),FieldGoal=mean(fg),Attemps=mean(fga))
#
#   Player by Field Goal Assist / games 2018
#
NBA20%>%group_by(player,pos,year) %>%filter(g>50)%>%
...
```
---
If i add a minutes played filter, does it change something

Filtering for more than 50 games and 1500 Minute played, plot the same caracteristics
and check if something change.
I assume that a player with a long time on the floor is a major team player

---
```{r statistics mp,echo=TRUE}
#
...
```
For a player set from 2018 with more then 50 games played:

calculate the average per 50 games and

Plot the correlation matrix by attributes :
...
```{r Player Info dataset selection,error=TRUE}
#
#Player info dataset
#
#ORpG: (Offensive Rebounds per Game): Average offensive rebounds a player made in a game. (ORB/G) 
...
fviz_contrib(player.pca, choice="var", axes = 1 )
# Contribution des variables à l'axe 2
fviz_contrib(player.pca, choice="var", axes = 2)
# Contribution des variables aux axes 1 + 2
fviz_contrib(player.pca, choice="var", axes = 1:2)
# Contribution des individus à l'axe 1
fviz_contrib(player.pca, choice="ind", axes = 1,top=20)
#
...
plotcluster(cluster_data, km.mod$cluster)
# More complex
clusplot(cluster_data, km.mod$cluster, color=TRUE, shade=TRUE, 
         labels=4, lines=0)
...
#
 ...
 PC2: (Assists per Game with Physical Capabilities
 Offensive player with good assist and offensive rebound capabilities
#
 PC3: (Steals per Game)
 Defensive player able to steal the ball
...
#
 ...
 The overall rebounder, focus on offensive and defensive rebound
#
PC7: (Offensive Rebounds per Game)-(Defensive Rebounds per Game)
 The overall rebounder, focus on offensive and defensive rebound
#
# 
PC8 : Physical Tall player
...
class.players %>% filter(km.cluster == 1)
class.players %>% filter(km.cluster == 2)
class.players %>% filter(km.cluster == 3)
class.players %>% filter(km.cluster == 4)
class.players %>% filter(km.cluster == 5)
class.players %>% filter(km.cluster == 6)
class.players %>% filter(km.cluster == 7)
...
```{r Player Full dataset analysis,error=TRUE}
...
#
# select data from fg to pts, scaling
#
...
##
# Contribution des variables à l'axe 1
fviz_contrib(nba.pca, choice="var", axes = 1 )
# Contribution des variables à l'axe 2
fviz_contrib(nba.pca, choice="var", axes = 2)
# Contribution des variables aux axes 1 + 2
fviz_contrib(nba.pca, choice="var", axes = 1:2)
# Contribution des individus à l'axe 1
fviz_contrib(nba.pca, choice="ind", axes = 1,top=20)
#
...
```{r Player Info dataset PCA,error=TRUE}
...
```{r Player Info dataset  Bayesian}
...
```{r Player Info dataset decision tree}
...
Classification using 18 abilities from NBA player dataset


```{r Player Classification analysis}

log.player <-P_info_all%+
  >%select(-player,-pos)
head(log.player)
...
#196 : 85.5< Weight <97.5, assis<177, blocking<15.8
#
P_196<-P_info_all%>% filter(p_kg>=85.5 & p_kg<97.5&ast<177 &blk<15.8)%>%mutate(pos=196)
...
```{r Player Machine learning,error=TRUE}
...
```{r Player analysis by Minutes Played}
...
nba_test<- NBA20%>%group_by(per)%>%filter(mp>1500)%>%select(player,pos,fg_pm,x3p_pm,x2p_pm,ft_pm,orb_pm,drb_pm,trb_pm,ast_pm,stl_pm,blk_pm,tov_pm,pts_pm,per,tov)
#
#   apply scaling
#
nba_test[, -c(1:2)] <- scale(nba_test[, -c(1:2)])
...
#
# pts effect
# Use test set,join FG averages & Pts averages
# Prediction equals the mean with user effect b_u & movie effect b_i
#
predicted_per_fgpts_norm <- validation %>% 
  left_join(fg_avgs_norm, by='fg_pm') %>%
  left_join(pts_avgs_norm, by='pts_pm') %>%
  mutate(pred = mu + b_fg + b_pts) 
...
predicted_per_reg <- validation %>% 
  ...
  mutate(pred = mu + r_fg + r_pts +r_orb+ r_drb + r_stl + r_blk + r_tov + r_pf + r_pd) %>% .$pred#
...
rmse_results <- bind_rows(rmse_results,
                          data_frame(method="Regularized Full Model",  
                                     RMSE = model_R_rmse ))
rmse_results %>% knitr::kable()

```

```{r Player PER RMSE with coefficient}
#
#
#  apply coefficient 
#  
...
predicted_per_reg <- validation %>% 
  ...
  left_join(r_pf, by = "x3p_pm") %>%
  left_join(r_pd, by = "x2p_pm") %>%
  mutate(pred = mu + r_fg + r_pts + r_drb*1.2 +r_stl*0.75 + r_blk + r_tov*0.75 + r_pf + r_pd + r_orb*1.2) %>% .$pred


model_R_rmse <- RMSE(validation$per,predicted_per_reg)
rmse_results <- bind_rows(rmse_results,
                          data_frame(method="Regularized Coeff Model",  
                                     RMSE = model_R_rmse ))
rmse_results %>% knitr::kable()
